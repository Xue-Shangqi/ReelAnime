...
class User:
    ...
    def AddToLikingList(self, anime_id, conn: psycopg.Connection):
        check_query = """
    ...
    def CreateUser(self, uname, password, conn: psycopg.Connection):
        self.username = uname
        self.password = password
        with conn.cursor() as cursor:
            queryCreate = """
    ...
    def GetAnimeInfo(self, anime_id, conn: psycopg.Connection):
            queryAnimeInfo = """
                SELECT * FROM anime
                WHERE anime_id = %s
            """
